...
exports.registerByAdmin = function (req, res) {
  ...
  UserValidator.validateCreate(req, function (err, data) {
    ...
    newUser.save(function (err, user) {
      if (err)
      {
        return ReponseHelper.response(res, 200, HashDataHelper.makeError({status: 422, msg: err.message}));
      }
      //var token = jwt.sign({_id: user._id}, config.secrets.session, {expiresInMinutes: 60 * 5});
      var data = user.toObject();
      return ReponseHelper.response(res, 200, HashDataHelper.make(data, 200));
...
exports.updateUser = function (req, res) {
  UserValidator.validateUpdate(req, function (err, data) {
    ...
    UserModel.findOne({_id: req.user._id, isDeleted: false}, function (err, docs) {
      ...
      {
        return uploadAvatar(req, res, function (error, reuslt) {
          if (error)
          {
            return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 422, error: err}));
          }
...
exports.updateUserasAdmin = function (req, res) {
  UserValidator.validateUpdate(req, function (err, data) {
    if (err) {
      return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 422, error: err}));
    }
    // if( req.user.role == 'admin'){
    UserModel.findOne({_id: data.id, isDeleted: false}, function (err, docs) {
      if (err) {
        return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 500, error: err}));
      }
      if (req.files != 'null' && typeof req.files.avatar != 'undefined') {
        return uploadAvatartoUser(req, res, function (error, reFile) {
          if (error) {
            return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 422, error: err}));
          }
          console.log(reFile);
          data.avatar = reFile.toObject();
          return updateProfile(docs, data, res);
        });

      } else {
        console.log('update user within avatar');
        return updateProfile(docs, data, res);
      }
...
exports.login = function (req, res) {
  UserValidator.validateLogin(req, function (err, data) {
    ...
    UserModel.findOne({email: data.email, isDeleted: false}, (err, result) => {
      if (!result) {
        return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 422, msg: 'Invalid email or Password', error: err}));
      }
      if (result.authenticate(data.password)) {
        ...
        return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 404, msg: 'Invalid email or Password'}));
      }
    })
  });
}
...
exports.allUsers = function (req, res) {
  ...
  UserModel.aggregate(pipe).then((totalDocs) => {
    UserModel.find({isDeleted: false, role: 'member'}).sort(sort).skip(skip).limit(limit).then((userDocs) => {
      ...
      return ReponseHelper.response(res, 200, {data: userDocs, total_count: totalDocs[0].total, status: 'success'});
    }).catch((errors) => {
      console.log(errors);
    })
...
exports.allModels = (req, res) => {
  ...
  pipe.push({"$group": total});
  UserModel.aggregate(pipe).then((totalDocs) => {
    UserModel.find({isDeleted: false, role: 'model'}).sort(sort).skip(skip).limit(limit).then((userDocs) => {
      delete result.access_token;
...
exports.changePassword = (req, res) => {
  if (req.body.user.id == req.user._id) {
    UserModel.find({id: req.user._id, isDeleted: false}, function (err, userDocs) {
      if (err) {
        return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 500, error: err}));
      }
      var newvalues = {$set: {
...
exports.updateUserasAdmin = function (req, res) {
  UserValidator.validateUpdate(req, function (err, data) {
    ...
    UserModel.findOne({_id: data.id, isDeleted: false}, function (err, docs) {
      ...
      if (req.files != 'null' && typeof req.files.avatar != 'undefined') {
        return ImageHelper.uploadAvatartoUser(req, res, function (error, reFile) {
          if (error) {
            return ReponseHelper.response(res, 422, HashDataHelper.makeError({status: 422, error: err}));
          }
          console.log(reFile);
...
function uploadAvatar(req, res, callback) {
