...
exports.getAllModels = function (req, res) {
  ...
  var condition = {role:'model',isActive:true};
...
exports.getSearch = (req,res)=>{
  ...
  //           });
  UserModel.find({"$text":{"$search":"/"+searchString+"/"},isActive:true,role:'model'}).limit(5).exec((err, docs) =>{ 
          if(err){
            console.log(err);
          }
          return ReponseHelper.response(res, 200, HashDataHelper.make(docs)); 
...
exports.deleteModel=function(req,res){
    var query=UserModel.update({_id:req.params.id,role:'model'},{$set:{isActive:false,isDeleted:true}});
